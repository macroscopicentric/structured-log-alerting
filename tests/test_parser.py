...
def test_parser_raises_on_attributeerror(
    malformed_log_line_b, correctly_formatted_log_line
):
...
def test_parser_parses_request_correctly(correctly_formatted_log_line):
    ...
    assert (
        "endpoint" in parsed_request._fields and parsed_request.endpoint == "/api/user"
    )
    assert (
        "http_version" in parsed_request._fields
        and parsed_request.http_version == "HTTP/1.0"
    )

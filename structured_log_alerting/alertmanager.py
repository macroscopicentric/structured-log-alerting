...
class AlertManager():
	...
	"""
	def __init__(self, counters_collection: CountersCollection, interesting_counter_names: list[str] = [], elevated_request_threshold: int = 10) -> None:
	...
	def find_highest_count(self, current_time: datetime = datetime.now(), since_interval_in_seconds: int = 10, metric_names: list[str] = []) -> str:
		"""
		...
		for metric in metric_names_to_check:
			temp_count: int = self.counters_collection.total_count_since(current_time, since_interval_in_seconds, metric)
	...
	def find_interesting_metrics_summaries(self, current_time: datetime = datetime.now(), since_interval_in_seconds: int = 10, metric_names: list[str] = []) -> list[str]:
		"""
		...
		try:
			...
			for metric in metric_names_to_check:
				count = self.counters_collection.total_count_since(current_time, since_interval_in_seconds, metric)
	...
	def provide_summary_for_interval(self, current_time: datetime = datetime.now(), since_interval_in_seconds: int = 10, interesting_metrics: list[str] = [], metrics_for_highest_count: list[str] = []) -> list[str]:
		"""
		Given a specific interval of time, provide sentence-length
		...
		summary_statements.append(f"Current time interval: {formatted_timestamp_for_printing}")

		summary_statements.append(self.find_highest_count(current_time, since_interval_in_seconds, metrics_for_highest_count))
		summary_statements.extend(self.find_interesting_metrics_summaries(current_time, since_interval_in_seconds, interesting_metrics))

		return summary_statements

	def check_for_elevated_requests(self, since_interval_in_seconds: int = 120) -> list[str]:
		"""
